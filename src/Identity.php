...
class Identity
{
    ...
    public static function checkUserComplete($scope)
    {
        ...
        try {
            ...
            if (self::isConnected()) {
                $userCompleted = OAuth::doCheckUserCompleted(self::$druid_config->getApiUrl('api', 'user'), $scope);
            }
        } catch (Exception $e) {
            self::$logger->error($e->getMessage());
        }
    ...
    public static function checkUserNeedAcceptTerms($scope)
    {
        ...
        try {
            ...
            if (self::isConnected()) {
                $status = OAuth::doCheckUserNeedAcceptTerms(self::$druid_config->getApiUrl('api', 'user'), $scope);
            }
        } catch (Exception $e) {
            self::$logger->error($e->getMessage());
        }
